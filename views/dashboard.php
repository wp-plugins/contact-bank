	...
	<div class="layout-span12">
		<div class="widget-layout">
			<div class="widget-layout-title">
				<h4><?php _e( "Dashboard - Contact Bank", contact_bank ); ?></h4>
			</div>
								...
								<table class="table table-striped" id="data-table-form">
									<thead>
										<tr>
											...
											<th style="width: 40%"><?php _e( "Shortcode", contact_bank ); ?></th>
											<th style="width: 30%"><?php _e( "Action", contact_bank ); ?></th>
										</tr>
									</thead>
									<tbody>
										<?php
										...
										{
											$id = $form_data[$flag]->form_id;
											?>
												<tr>
													<td><?php echo $form_data[$flag]->form_name;?></td>
													<td><?php echo "[contact_bank form_id=".$form_data[$flag]->form_id."]";?></td>
													<td><a href="admin.php?page=edit_contact_view&id=<?php echo $id;?>" class="btn" data-original-title="<?php _e("Edit",contact_bank)?>">
													<i class="icon-pencil"></i>
													</a>
													<a herf="#" onclick="delete_form(<?php echo $id;?>)" class="btn" data-original-title="<?php _e("Delete",contact_bank)?>">
...
function delete_form(dynamicId)
{
	var checkstr =  confirm("<?php _e( "Are you sure to delete this form ?", contact_bank ); ?>");
	if(checkstr==true)
	{
		jQuery.post(ajaxurl,"id="+dynamicId+"&param=delete_form&action=add_contact_form_library",function(data)
		{
			location.reload();
		});
	}
...
{
	var checkstr =  confirm("<?php _e( "Are you sure to delete all forms ?", contact_bank ); ?>");
	if(checkstr==true)
	{
